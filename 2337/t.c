...
void addedge(int u,int v,int index)
{
    ...
    printf("tot:%d next:%d head[%d]:%d\n", tot-1, edge[tot-1].next, u, head[u]);
}
string str[1010];
int in[30],out[30];
int cnt;
...
void ShowEdges(int num)
{
    int index;

    for(index = 0; index < num; index++)
    {
        cout<<"Edge["<<index<<"]: "<<str[edge[index].index]<< " next:";
	if(edge[index].next == -1)
	    printf("-1 ");
	else
	    cout<<str[edge[head[edge[index].next]].index];
	printf("\n");
    }
}
int main()
